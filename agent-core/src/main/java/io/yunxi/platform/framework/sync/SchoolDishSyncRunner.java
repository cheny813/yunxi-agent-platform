 ...
 *
 * <p>
 * 替代旧的 SchoolDishVectorSyncService，管理公共菜品库和各学校专属菜品库的同步。
 * </p>
 *
 * <p>
 * 同步策略：
...
public class SchoolDishSyncRunner implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        if (!milvusOps.isAvailable()) {
            log.warn("SchoolDishSyncRunner: Milvus 不可用，跳过学校菜品同步");
            return;
        }
     ...
     * <li>再查询所有学校，逐个同步学校专属菜品</li>
     * </ol>
     */
    private void syncAllSchoolDishes() {
        try {
            // 1. 同步公共菜品库
            log.info("SchoolDishSyncRunner: 同步公共菜品库");
            syncSchoolDishes(null);

            // 2. 查询所有学校
            List<Long> schoolIds = queryAllSchoolIds();
            log.info("SchoolDishSyncRunner: 查询到 {} 个学校", schoolIds.size());

            // 3. 逐个同步学校专属菜品
            for (Long schoolId : schoolIds) {
                try {
                    syncSchoolDishes(schoolId);
                    Thread.sleep(1000); // 每个学校间隔1秒，避免压力过大
                } catch (Exception e) {
                    log.error("同步学校 {} 专属菜品失败", schoolId, e);
                }
            }

            log.info("========== SchoolDishSyncRunner: 学校菜品同步完成 ==========");
        } catch (Exception e) {
            log.error("SchoolDishSyncRunner: 同步失败", e);
        }
    }
    ...
    private void syncSchoolDishes(Long schoolId) {
        ...
        log.info("  开始同步 {} -> 集合 {}", taskDesc, collectionName);

        try {
            // 1. 确保集合存在
            ensureCollectionExists(collectionName);

            // 2. 集合已有数据则跳过（避免每次启动重复同步）
            if (milvusCollectionService.getCollectionCount(collectionName) > 0) {
                log.info("  {} 已有数据，跳过", taskDesc);
                return;
            }
            ...
            if (dishes.isEmpty()) {
                log.info("  {} 无数据，跳过", taskDesc);
                return;
            }
            log.info("  查询到 {} 个菜品", dishes.size());
            ...
            log.info("  {} 同步完成: {} 条", taskDesc, dishes.size());
        } catch (Exception e) {
            log.error("  {} 同步失败", taskDesc, e);
        }
    }
    ...
    private List<JsonObject> buildDishDataList(
        ...
        return dishes.stream().map(dish -> {
            ...
            // embedding 文本（稍后由 insertToMilvus 填充向量）
