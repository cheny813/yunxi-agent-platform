...
import io.yunxi.platform.knowledge.KnowledgeCreator;
 ...
 *
 * <p>
 * 基于 {@code agentscope.yml} 中的 {@code agentscope.extensions.knowledge-bases}
 * 配置，自动创建并注册 Knowledge Bean 到 Spring 容器。
 * 每种知识库类型通过 {@link KnowledgeCreator} SPI 接口扩展。
 * </p>
 *
 * <h3>V2.0 说明</h3>
 * <p>
 * {@link io.agentscope.core.rag.Knowledge} 在 AgentScope 2.0.0 中标记为弃用，
 * 新 RAG 模块计划在后续 minor 版本上线。届时 KnowledgeAutoConfiguration
 * 需适配新的知识库 SPI 接口。
 * </p>
 *
 * <h3>工作原理</h3>
 ...
 * </p>
 * <ol>
 * <li>在 {@code agentscope.yml} 中添加配置模块</li>
...
// TODO: AgentScope 2.0 新 RAG 模块上线后，适配新的知识库 SPI 接口，替换 registerKnowledgeBean 逻辑
...
public class KnowledgeAutoConfiguration {
     ...
     * 使用对应 {@link KnowledgeCreator} 创建实例并注册为 Spring Bean。
     * </p>
     */
    @PostConstruct
    public void registerKnowledgeBeans() {
        ...
        if (knowledgeBases == null || knowledgeBases.isEmpty()) {
            log.info("No knowledge base configured, skip auto registration");
            return;
        }
        ...
        for (Map.Entry<String, KnowledgeBaseConfig> entry : knowledgeBases.entrySet()) {
            ...
            if (!config.isEnabled()) {
                log.debug("Knowledge base disabled: {} (type={})", configKey, config.getType());
                continue;
            }
            ...
            try {
                ...
                log.error("Register knowledge base failed: {} (type={})", configKey, config.getType(), e);
            }
        }

        log.info("Knowledge base auto-registration completed: {} registered", registeredCount);
    }
