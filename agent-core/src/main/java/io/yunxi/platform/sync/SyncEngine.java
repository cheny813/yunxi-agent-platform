...
public class SyncEngine implements InitializingBean {
     ...
     * 注入管道配置列表
     * <p>
     * 由 {@link io.yunxi.platform.sync.config.SyncPipelineConfigLoader}
     * 在初始化时调用
     * </p>
     */
    ...
    public void executePipeline(SyncPipelineConfig cfg) {
        ...
        if (incremental == null || incremental.getColumn() == null) {
            if (milvusCollectionService != null) {
                ...
                if (count < 0) {
                    log.warn("集合 {} 查询失败（返回负数 {}），跳过同步管道: {}（保守策略）",
     ...
     * 构建 Milvus 插入数据
     * <p>
     * 将转换后的数据 + 向量转换为 Milvus SDK 所需要的 JsonObject 列表
     * </p>
     */
    ...
    /**
     * 将配置的类型字符串映射为 DataType 枚举（公共静态方法，供 MilvusSchemaInitializer 复用）
     */
     ...
     * 读取持久化游标值
     * <p>
     * 从数据库读取管道上次同步游标；不存在时返回 initialValue（首次增量同步）
     * </p>
     */
