 ...
 *
 * <p>
 * 负责 ingredient_classes 集合的数据解析、向量化、插入和增量同步。
...
@Slf4j
@Component
public class IngredientClassesSyncHandler implements SyncHandler {

    /** Milvus 集合名称 */
    private static final String COLLECTION_NAME = "ingredient_classes";
     ...
     */
    public IngredientClassesSyncHandler(
     ...
     * 执行食材分类增量同步
     *
     * <p>从 MCP 数据库查询食材分类数据，解析后向量化并 upsert 到 Milvus。</p>
     */
    @Override
    public void sync() {
        log.info("开始增量同步食材分类...");
        try {
            String sql = "SELECT fic.id, fic.main_id, fic.sub_id, " +
                    "fimc.name AS main_class_name, fisc.name AS sub_class_name " +
            ...
            if (items.isEmpty()) {
                log.info("食材分类数据为空，跳过");
                return;
            }

            insertClassData(items);
            log.info("食材分类增量同步完成，共 {} 条", items.size());
        } catch (Exception e) {
            log.error("食材分类增量同步失败", e);
        }
    }
    ...
    private void insertClassData(List<Map<String, Object>> items) {
        ...
        for (Map<String, Object> item : items) {
            ...
            String text = String.format("食材分类 %s", className);
            texts.add(text);
        }
