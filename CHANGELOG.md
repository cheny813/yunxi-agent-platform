...
- **[Unreleased]**：`ModelFactory` 由 1 参 `registerFactory` 改为 2 参 `ContextModelFactory`，各官方提供商统一经 `ModelCreationContext` 透传覆盖值；清理死代码 `registerModelWithOptions`；新增 `LlmMetrics.recordAndLogUsage` 统一入口并落地于 `ChatAppService` / `PageAgentService`；`application.yml` 增加 `io.agentscope.core.model: DEBUG` 开关。
